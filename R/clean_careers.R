...
clean_careers <- function(x) {
    ...
    # dplyr::lag the EMPNO variable by 1 position
    combined <- x %>% dplyr::mutate(EMPNO2 = dplyr::lag(EmployeeID, n = 1))  
    
    # dplyr::lag the ITEM variable by 1 position
    combined <- combined %>% dplyr::mutate(ITEM2 = dplyr::lag(TitleCode, n = 1))  
    
    # dplyr::lagging shifted item numbers that didn't necessarily belong to the next person in the data.
    # Identify the records where the dplyr::lagged EMPNO2 is not equal to EMPNO and convert their ITEM2 values to NA
    combined[which(combined$EMPNO2 != combined$EmployeeID),]$ITEM2 <- NA  
    
    # dplyr::rename title code variables
    combined <- combined %>%
    ...
    # Create a new variable called "JobApptDate1" and give it the dplyr::lagged job appt dates. 
    # Use JobApptDate1 because it was the starting (or previous) item for the person
    combined <- combined %>% 
        dplyr::mutate(JobApptDate1 = dplyr::lag(JobApptDate, n = 1)) 
    
    # dplyr::rename JobApptDate to JobApptDate2 which indicates it is the next item the person held.
    combined <- combined %>%
    ...
    combined_exp <- combined_exp %>%
        dplyr::mutate(TitleLong = ifelse(!is.na(TitleLong),
                                  TitleLong,
                                  ifelse( is.na(TitleLong),
                                          ImputedTitle,
                                          NA))) %>%
        dplyr::select(-ImputedTitle)
    
    
    # Identify expired items in the Item1 column
    combined_exp <- combined_exp %>% 
        dplyr::mutate(Item1_exp = ifelse( !Item1 %in% current_titles$TitleCode & !is.na(Item1),
                                  1, 0))
    
    # Identify expired items in the Item2 column
    combined_exp <- combined_exp %>%
        dplyr::mutate(Item2_exp = ifelse( !Item2 %in% current_titles$TitleCode,
                                  1, 0))
    
    # Identify expired items in EITHER Item1 or Item2
    combined_exp <- combined_exp %>%
        dplyr::mutate(Any_item_exp = ifelse(Item1_exp == 1 | Item2_exp == 1,
                                     1, 0))
    ...
    combined_exp$Item2copy <- stringr::str_sub( combined_exp$Item2, 1, 4)  # temp copy
    combined_demotions <-
        dplyr::left_join(combined_exp, salary[, c("Item", "Range Max")],
                  by = c("Item2copy" = "Item")) %>%
        dplyr::rename(Salary2 = `Range Max`) %>%
        dplyr::select(-Item2copy)
    
    combined_demotions$Item1copy <- stringr::str_sub( combined_demotions$Item1, 1, 4)  # temp copy
    combined_demotions <- dplyr::left_join(combined_demotions, salary[,c("Item", "Range Max")], 
                                    by = c("Item1copy" = "Item")) %>%
        dplyr::rename(Salary1 = `Range Max`) %>%
        dplyr::select(-Item1copy)
    
    # Create a new variable called "Demotion" and assign 1 (demotion) or 0 (non-demotion)
    combined_demotions <- combined_demotions %>% 
        dplyr::mutate(Demotion = ifelse(Salary2 == Salary1,        # if salaries are equal
                                 0,                         # assign 0
    ...
    combined_demotions <- combined_demotions %>% 
        dplyr::mutate(greater5 = ifelse( (Salary1-Salary2)/Salary1 > 0.0550000,
    ...
    combined_demotions <- combined_demotions %>%
        dplyr::select(EmployeeID, Item2, TitleLong, JobApptDate2, JobLastDate,   
