...
make_item_ref <- function(x) {
  ...
  ref <- x %>% dplyr::select(Item1, Item1Name, Salary1, Salary1Min,
                             Hyperlink)
  ...
  # dplyr::rename the columns
  ref <- ref %>%
    dplyr::rename(TitleCode = Item1, TitleLong = Item1Name, SalaryMax = Salary1, SalaryMin = Salary1Min)
  
  ref <- dplyr::left_join(ref, incumbents,
                          by = c("TitleCode" = "TitleCode") )
  
  ref <- ref %>% 
    dplyr::rename(Incumbents = Count)
  
  ref[ is.na(ref$Incumbents), "Incumbents"] <- 0
  
  # Reorder columns
  ref <- ref %>% 
    dplyr::select(TitleCode, TitleLong, SalaryMax, SalaryMin, Incumbents, Hyperlink)
