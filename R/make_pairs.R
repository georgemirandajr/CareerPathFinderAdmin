...
make_pairs <- function(x) {
    ...
    item_pairs <- item_pairs %>%
        rename(Item1 = Var1, Item2 = Var2, PairCt = Freq)
    
    # Calculate the probability of each pair in item_pairs
    item_pairs <- item_pairs %>%
        mutate(Prob = round(PairCt / ItemCt, 5)) %>%
        arrange(Item1, -Prob, -PairCt)  # decreasing order by item, probability, then by count of each pair
    ...
    item_pairs <- item_pairs %>%
        select(Item1, Item1Name, Item2, Item2Name, PairCt, ItemCt, Prob, 
